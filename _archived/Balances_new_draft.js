...
function rebuildBalancesFromHistory() {
  try {
    ...
    // Name(0), Type(1), Number(2), Bank(3), Balance(4), LastUpdate(5), IsMine(6), Int(7), Alias(8)
    var accData = accSheet.getRange(2, 1, accLast - 1, 9).getValues();
    ...
    for (var i = 0; i < accData.length; i++) {
        ...
        aliases.forEach(function(a){ if(a.trim()) indexMap[a.trim()] = i; });
        
        // Reset starting balance effectively to 0 for rebuild
        balancesByIndex[i] = 0.0;
    }
    ...
    for (var k = 0; k < txData.length; k++) {
      ...
      if (idx !== -1) {
          if (isIncoming) balancesByIndex[idx] += amount;
          else balancesByIndex[idx] -= amount;
      }
    }
    ...
    return 'Rebuild Successful (Optimized)';
